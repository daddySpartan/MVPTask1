...
function SaleData(props) {
  const { option, open, openSaleModal, fetchSale, slId, sldata1, sldata2} = props;

  const [cid, setCid] = useState("");
  const [pid, setPid] = useState("");
  const [sid, setSid] = useState("");
  const [salesDate, setSalesdate] = useState(sldata2);
        ...
        <Modal.Content>
          <Modal.Description>
            ...
            <Form.Field>
                ...
                <input 
                placeholder={sldata2} //'Change date sold (MM/dd/yyyy)'
                onChange = { (e)  => setSalesdate(e.target.value)}/>              
              </Form.Field>
             <Form.Field>
                <label>Customer Name</label>
                <Select placeholder={sldata1}options={cList} onChange={(e, data) => handleSelect1(data.value)}/>                  
              </Form.Field>
              <Form.Field>
                <label>Product Name</label>
                <Select placeholder={getSalebyId(slId,3)} options={pList} onChange={(e, data) => handleSelect2(data.value)}/>                  
              </Form.Field>
              <Form.Field>
                <label>Store Name</label>
                <Select placeholder={getSalebyId(slId,4)} options={sList} onChange={(e, data) => handleSelect3(data.value)}/>                  
              </Form.Field>
            </Form>
          </Modal.Description>
        </Modal.Content>
